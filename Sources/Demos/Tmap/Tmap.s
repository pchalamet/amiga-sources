...
COP_SKIP=28*4
COP_SIZE_X=(1+NB_COLONNES+4)*4
COP_SIZE=COP_SKIP+COP_SIZE_X*NB_LIGNES+4

PIXEL_SIZE=3
SCREEN_X=NB_COLONNES*PIXEL_SIZE
SCREEN_Y=NB_LIGNES*PIXEL_SIZE
SCREEN_DEPTH=8
...
*********************************************************************************
	...
	lea _DataBase,a5			on precalcule tout ça !
	bsr Global_Init
	bsr Build_Screen
	bsr Build_Coplists
...
*********************************************************************************
Build_Screen
	move.l #Screen_space,d0			ecran sur multiple de 8
	addq.l #4,d0
	and.l #~$7,d0
	move.l d0,Screen(a5)
	move.l d0,a0
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Build_Motif
	moveq #1,d0				on commence à la couleur 1
	bsr.s Build_Motif_Lines
	bsr.s Dup_Motif_Lines
	move.w #129,d0				puis couleur 128
	bsr.s Build_Motif_Lines
	bsr.s Dup_Motif_Lines

	IFNE CANAL_PLUCHE
* décalage vers la gauche de 1 pixel des lignes paires
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Canal_Plus_Motif
	move.l Screen(a5),a0			on décale les lignes paires de 1 pixel à gauche
	lea SCREEN_WIDTH*SCREEN_DEPTH(a0),a0	on fait ca de gauche à droite
	move.w #(2*PIXEL_SIZE)/2-1,d0
do_canal
	move.l a0,a1
	move.l a0,a2
	move.w #(SCREEN_WIDTH*SCREEN_DEPTH)/4-1,d1
	moveq #0,d2				carry cleared
.canal
	addx.l -(a1),-(a2)
	dbf d1,.canal

	lea SCREEN_WIDTH*SCREEN_DEPTH*2(a0),a0	ligne paire suivante
	dbf d0,do_canal
	ENDC

* remplissage de tout l'écran pour creer le motif final
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Fill_Screen_Width_Motif
	move.l Screen(a5),a0
	lea SCREEN_WIDTH*SCREEN_DEPTH*2*PIXEL_SIZE(a0),a0
	move.l Screen(a5),a1			Début du motif
	move.w #((NB_LIGNES-2)*SCREEN_WIDTH*SCREEN_DEPTH*PIXEL_SIZE)/4-1,d0
.Dup
	move.l (a1)+,(a0)+
	dbf d0,.Dup
	rts
...
* Duplication de la ligne qui vient juste d'etre construite
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*   -->	a0=Ecran
* <--	a0=Ligne*PIXEL_SIZE suivante
Dup_Motif_Lines
	lea SCREEN_WIDTH*SCREEN_DEPTH(a0),a1
	exg a0,a1				a0=Destination    a1=Source
	move.w #(SCREEN_WIDTH*SCREEN_DEPTH*(PIXEL_SIZE-1))/4-1,d0
.Dup
	move.l (a1)+,(a0)+
	dbf d0,.Dup
	rts
...
.branch
	...
	moveq #0,d2
	move.w d1,d2
	addq.w #1,d1				PosX=PosX+1
	divu #PIXEL_SIZE,d2
	swap d2
	tst.w d2
	bne.s .skip
	addq.w #1,d0				couleur suivante
.skip	cmp.w #SCREEN_X+1,d1
...
Build_One_Coplist
	...
	move.l #(bplcon0<<16)|($0210),(a0)+
	move.l #(bplcon1<<16),(a0)+
	move.l #(bplcon2<<16),(a0)+
	move.l #(bplcon3<<16),(a0)+
	move.l #(ddfstrt<<16)|($0038),(a0)+
	move.l #(ddfstop<<16)|($a0),(a0)+
	move.l #(diwstrt<<16)|($2e81),(a0)+
	move.l #(diwstop<<16)|($27c1),(a0)+
	move.l #(bpl1mod<<16)|(SCREEN_WIDTH*(SCREEN_DEPTH-1)),(a0)+
	move.l #(bpl2mod<<16)|(SCREEN_WIDTH*(SCREEN_DEPTH-1)),(a0)+
...
Table_Zoom		rs.b 112726			euh..
Table_Rotate_Offset	rs.l 360
Table_Rotate		rs.b 235200			euh...
...
Coplist_space
	ds.b COP_SIZE*3
Screen_space
	ds.b SCREEN_WIDTH*SCREEN_Y*SCREEN_DEPTH+4
org_coplist

...
COP_SKIP=33*4
COP_SIZE_X=(1+1+NB_COLONNES+4)*4
...
Build_One_Coplist
	...
	move.l #(diwstrt<<16)|($5e80),(a0)+
	move.l #(diwstop<<16)|($0f00)|(($80+SCREEN_X)&$ff),(a0)+
...
** on arrive à la partie suivante par un COP_SKIP sur un pointeur coplist
	...
	move.l #($0001fffe)|(($5e-PIXEL_SIZE_Y)<<24),d5
Build_All
...
.loop_all
	lea 256*2(a1),a2			saute la palette
	move.w #TEXTURE_X*TEXTURE_Y-1,d1
.loop_texture
	moveq #0,d2
	move.b (a2)+,d2				lit l'octet chunky
	move.w (a1,d2.w*2),d2			convertit en couleur WORD  $xyz
	move.w d2,(a0)+				texture shadées #0

* fabrication autres textures shadées
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	lea TEXTURE_X*TEXTURE_Y*2-2(a0),a3	-2 car (a0)+
...
.skip
	move.w d4,(a3)
	lea TEXTURE_X*TEXTURE_Y*2(a3),a3
	move.w d4,d2
	dbf d3,.build_shade

	dbf d1,.loop_texture

	move.l a2,a1				texture suivante
	lea TEXTURE_X*TEXTURE_Y*2*(TEXTURE_SHADE-1)(a0),a0
	dbf d0,.loop_all
	rts
...
.loop_case
	...
	beq.s .no_texture
	subq.w #1,d4
	mulu.w #TEXTURE_X*TEXTURE_Y*2*TEXTURE_SHADE,d4
	lea (a2,d4.l),a3
	move.l a3,(a0)+
	lsr.w #4,d3
	dbf d2,.loop_case
...
Build_Table_Screen_Offset
	...
	moveq #4+4+4+2,d0			wait/bplcon4/bplcon3/move
	moveq #31-1,d1
...
Build_Table_Zoom
	...
	lea TEXTURE_X*TEXTURE_Y*2*(TEXTURE_SHADE-1),a3
...
* Génaration de la table de saut pour chaque zoom
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	move.l a1,d4
	sub.l a0,d4
	subq.l #2,d4
...
* Repositionnement dans la texture en fonction de l'éloignement
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	cmp.w (a2),d0
	blt.s .no_next
...
* fabrique le code pour zoomer une colonne
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	moveq #0,d4
.For_A
	tst.w d1
...
Table_Zoom		rs.b 78512			cf MONAM !

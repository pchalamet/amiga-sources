...
NB_LINE=12
NB_MOVEM=136
NB_GAIN=(14*4*NB_MOVEM)/(SCREEN_WIDTH/8)
INC_ZOOM=8
CHANGE_ZOOM=500
MIN_ZOOM=300
GLAZNOTZ=0
...
vbl
	...
	move.b #$6f,front_back-data_base(a5)
	cmp.w #CHANGE_ZOOM,Zoom-data_base(a5)
	bgt.s no_change
	move.b #$6c,front_back-data_base(a5)
no_change
	bsr.s Flip_Screen
	bsr Compute_Matrix
	bsr Compute_Dots
...
Flip_Screen
	...
	move.w #((2*SCREEN_HEIGHT-NB_GAIN)<<6)|(SCREEN_WIDTH/16),bltsize(a6)

	cmp.w #MIN_ZOOM,Zoom-data_base(a5)
	ble.s no_zoom
	subq.w #INC_ZOOM,Zoom-data_base(a5)
no_zoom
	rts
...
Fill_Screen
	...
	lea (SCREEN_WIDTH/8)*SCREEN_HEIGHT*NB_BPL-2(a0),a0
	move.l a0,bltapt(a6)
	move.l a0,bltdpt(a6)
	clr.l bltamod(a6)
	move.l #$09f00012,bltcon0(a6)
	move.w #(SCREEN_HEIGHT<<7)|(SCREEN_WIDTH/16),bltsize(a6)
	rts

*---------------------------> incrementation des angles de rotations
Incrize_Angles
	lea Alpha(pc),a0
do_Alpha
...
*------------------------> recherche les droites qui doivent etre affichées
Compute_Lines
...
Display_Lines
	move.l log_screen(pc),a0
	lea (SCREEN_WIDTH/8)*SCREEN_HEIGHT*NB_BPL(a0),a6
	moveq #0,d0				efface quelques registres !!
	moveq #0,d1
	moveq #0,d2
	moveq #0,d3
	moveq #0,d4
...
loop_display_lines
	...
	movem.w 0(a2,d0.w),d0-d1
	movem.w 0(a2,d2.w),d2-d3
	lea (SCREEN_WIDTH/8)*SCREEN_HEIGHT(a3),a0
	exg a4,a5
	bsr.s Line
	exg a4,a5
...
not_in_all_bpl
	...
	rts
not_in_bpl1
	lea (SCREEN_WIDTH/8)*SCREEN_HEIGHT(a0),a0	trace ds le bpl2
	exg a4,a5
	bsr.s Line
	exg a4,a5
	dbf d7,loop_display_lines
...
*-----------------------------> routine qui trace les routines de droites
Draw_RightLines
...
*-----------------------------> routine de tracé de droite + clipping
...
*---------------> routine speciale pour trace des droites vertivcales à droites
...
.no_inter_Y_min
	...
	cmp.w #SCREEN_HEIGHT-1,d0
	bgt.s no_right_line
	move.w #SCREEN_HEIGHT-1,d1
.no_inter_Y_max
	sub.w d0,d1				récupère le DeltaY		
	add.w d0,d0
	move.w 0(a0,d0.w),d0			mulu #(SCREEN_WDITH/8),d0
	lea (SCREEN_WIDTH/8)-2(a3,d0.w),a1	adr de départ

	lsl.w #6,d1				calcul de bltsize
	addq.w #1,d1

wait_blitter
	btst #6,dmaconr(a6)			init le blitter
	bne.s wait_blitter
	move.l a1,bltcpt(a6)
	move.l a1,bltdpt(a6)
	move.w #(SCREEN_WIDTH/8)-2,bltcmod(a6)
	move.w #(SCREEN_WIDTH/8)-2,bltdmod(a6)
...
*---------------------> routine d'init pour le trace de droite avec le blitter
LineInit
	IIF GLAZNOTZ move.w #$f0f,color00(a6)
	btst #6,dmaconr(a6)
	bne.s LineInit
	IIF GLAZNOTZ move.w #$000,color00(a6)
...
DIST=500
Dots_3d
	dc.w DIST,DIST,DIST			point 0
	dc.w DIST,-DIST,DIST			point 1
	dc.w -DIST,-DIST,DIST
	dc.w -DIST,DIST,DIST
	dc.w DIST,DIST,-DIST
	dc.w DIST,-DIST,-DIST
	dc.w -DIST,-DIST,-DIST
	dc.w -DIST,DIST,-DIST			point 7
...
coplist
	...
	section ecran,bss_c
screen1
	ds.b (SCREEN_WIDTH/8)*SCREEN_HEIGHT*NB_BPL
screen2
	ds.b (SCREEN_WIDTH/8)*SCREEN_HEIGHT*NB_BPL

...
Add_File
	...
	lea FileRequest_Tags(pc),a1		par AslRequest()
...
Add_Dir
	...
	lea DirRequest_Tags(pc),a1		par AslRequest()
	bsr Replace_Requester
	CALL _AslBase(a5),AslRequest
	tst.l d0
	beq .no_add
...
* Package de la liste Selected
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Pack
	st Flag_Work_Pack(a5)

	clr.l Current_Password(a5)		ca chercher le password si yen a un
	tst.b StringSet_Tags+4+3-_DataBase(a5)
	bne.s .no_password

	move.l Gadget_String(a5),a0
	move.l gg_SpecialInfo(a0),a0
	move.l si_Buffer(a5),Current_Password(a5)
.no_password
...
* Depackage de la liste Selected
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Unpack
...
.file
	...
	move.l a0,Pack_OutName-_DataBase(a5)
	move.l Current_Password(a5),Pack_Password-_DataBase(a5)
	lea Pack_Tags(pc),a0
	move.l _XpkBase(a5),a6

	tst.b Flag_Work_Pack(a5)		un pack ou on dépack ?
	beq.s .unpack
.pack
	CALL XpkPack
	bra.s .done
...
.no_more
	...
	CALL UnLock
.no_lock
	addq.l #4,sp				bouffe le lock placé sur la pile
	rts
...
Display_About
	...
	lea xPKWin_EasyRequest_About(pc),a1
	sub.l a2,a2
	sub.l a3,a3
...
* ~~~~~~~~~~~~
	...
	dc.w CHECKIT|MENUTOGGLE
	dc.l %10000,Toggle_PackInfo

	dc.b NM_ITEM,0
	dc.l PackExecOnly_Str,PackExecOnly_Key
	dc.w CHECKIT|MENUTOGGLE
	dc.l %1000,Toggle_PackExecOnly
...
xPKWin_EasyRequest_About
	dc.l es_SIZEOF
	dc.l 0
	dc.l EasyRequest_RequestName
	dc.l EasyRequest_About
...
EasyRequest_About	dc.b "        xPKWin v"
			dc.b "0"+VERSION,"."
			dc.b "0"+REVISION
			dc.b " ©1994 Pierre Chalamet",10
			dc.b "           *** UNREGISTRED VERSION ***",10
			dc.b 10
			dc.b "      *** xPKWin is a SHAREWARE product ***",10
			dc.b "If you use it regularly, I invite you to register.",10
			dc.b " Just fill up the registration card and send your",10
			dc.b "   donation of at least 50 FF / 50$ US / 20 DM",10
			...
			dc.b "                    France",10
			dc.b 10
			dc.b "   NO PROFIT MUST BE DONE BY SELLING THIS TOOL.",0

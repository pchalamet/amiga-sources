...
MAKE_NUMBER2	macro
	IFNE \2
NUMBER\@ set (\1-((\1/10)*10))
		MAKE_NUMBER2 (\1/10),(\2-1)
		IFNE NUMBER\@
		dc.b "0"+NUMBER\@
ZERO set 1
		ENDC
...
MAKE_VERSION	macro
	MAKE_NUMBER MAJOR_VERSION_NUMBER
	dc.b "."
	MAKE_NUMBER VERSION_NUMBER
	dc.b "."
	MAKE_NUMBER REVISION_NUMBER
	endm
...
Main
	bra.s Skip_Version
	dc.b "$VER:"
	dc.b "IFF-Converter v"
	MAKE_VERSION
	dc.b " ©1993-1995 Sync/DreamDealers",0
...
* Affichage de l'espacement de la grille
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	lea InfoText3+44-db(a5),a0
	move.w Grid_Spacing-db(a5),d0
	moveq #4-1,d1
	bsr Write_Decimal_Number
...
* Affichage d'infos concernant la brosse
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Return_Flush_Raw_Normal
	moveq #0,d3
	moveq #7,d7				on traite par octets
...
Save_Raw_Normal_Next_Pixel
	...
	move.b d3,(a3)+				sauve dans le buffer
	subq.l #1,d2
	bne.s Save_Raw_Normal_Line
	bra Flush_Raw_Normal_Buffer
...
**************************************
* met sur le multiple demandé
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~
	move.w Save_Right-db(a5),d0
	sub.w Save_Left-db(a5),d0
	moveq #0,d1
	move.w d0,d1
	addq.w #8,d0
	lsr.w #3,d0				nb d'octets écrits

	add.w Grid_Spacing-db(a5),d1
	lsr.w #3,d1				nb d'octets à écrire
...
*****************************************************************************
********************                                      *******************
******************** MODIFICATION DU SPACING DE LA GRILLE *******************
********************                                      *******************
*****************************************************************************
SetSpacing8
	moveq #8,d7
...
ScreenTitle		dc.b "IFF-Converter v"
			MAKE_VERSION
			dc.b 0
ChildScreenTitle	dc.b "IFF-Converter Control Screen",0

Request_Welcome		dc.b "IFF-Converter v"
			MAKE_VERSION
			dc.b 10
...
GridSpacingStr		dc.b "Set Grid Spacing",0
Spacing8Str		dc.b "to 8 bytes",0
Spacing16Str		dc.b "to 16 bytes",0
Spacing32Str		dc.b "to 32 bytes",0
Spacing64Str		dc.b "to 64 bytes",0

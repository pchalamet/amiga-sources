...
Main
	bra.s Skip_Version
	dc.b "$VER:"
	dc.b "IFF-Converter v1.0.23  (c)1993-1994 Sync of DreamDealers",0
	even
...
.ok2
	...
	moveq #0,d0
	move.w Tag_Screen_Width(pc),d0
	moveq #0,d1
	move.w Tag_Screen_Height(pc),d1
	moveq #0,d2
	move.w Tag_Screen_Depth(pc),d2
	moveq #BMF_CLEAR!BMF_DISPLAYABLE!BMF_INTERLEAVED,d3
	sub.l a0,a0
	CALL AllocBitMap
	move.l d0,Tag_Window_BitMap-db(a5)
	beq no_allocbitmap_window
	NO_ERROR
...
.no4	move.l Tag_Screen_BitMap(pc),d0		ferme le vieux bitmap de l'écran
	beq.s .no5
	move.l d0,a0
	CALL _GfxBase(pc),FreeBitMap

...
Main
	bra.s Skip_Version
	dc.b "$VER:"
	dc.b "IFF-Converter v1.0.26  (c)1993-1995 Sync of DreamDealers",0
...
no_allocmem
	...
	CALL CloseScreen
no_openscreen
	move.l Tag_Window_BitMap(pc),a0		libère la bitmap de la fenetre
	CALL _GfxBase(pc),FreeBitMap
no_allocbitmap_window
	move.l Tag_Screen_BitMap(pc),a0		libère la bitmap de l'écran
	CALL FreeBitMap
...
Save_Raw_Normal_Next_Pixel
	addq.w #1,d4				yen a encore a sauver sur la
	cmp.w Save_Right-db(a5),d4		ligne ?
	bgt.s Save_Raw_Normal_Next_Line
	dbf d7,Save_Raw_Normal_Word
* sauvegarde dans le buffer
	move.w d3,(a3)+				sauve dans le buffer
	subq.l #1,d2
	beq.s Flush_Raw_Normal_Buffer

Return_Flush_Raw_Normal
Save_Raw_Normal_Next_Line
...
.no2	move.l Screen_Handle(pc),d0		ferme le vieil ecran
	beq.s .no3
	move.l d0,a0
	CALL CloseScreen
	clr.l Screen_Handle-db(a5)

.no3	move.l Tag_Window_BitMap(pc),d0		ferme le vieux bitmap de la fenetre
	beq.s .no4
	move.l d0,a0
	CALL _GfxBase(pc),FreeBitMap
...
.no4
;	move.l Tag_Screen_BitMap(pc),d0		ferme le vieux bitmap de l'écran
;	beq.s .no5
;	move.l d0,a0
;	CALL _GfxBase(pc),FreeBitMap
...
IFF_Menus
	...
	dc.w 0
	dc.l 0,Init_Save_Chunky_Picture
	
	dc.b NM_SUB,0
	dc.l RawNormalPicStr,RawNormalPicStr
	dc.w 0
	...
	dc.w 0
	dc.l 0,Init_Save_Chunky_Brush
	
	dc.b NM_SUB,0
	dc.l RawNormalBrushStr,RawNormalBrushKey
	dc.w 0

...
*		The Module Converter v4.1   ©1993 Sync/DRD
*		----------------------------------------------->
*			Last change : 2 Aout 1993
...
Entry_Point
	...
	dc.b "$VER: TMC v4.1 ©1993 Sync of DreamDealers",0
	even
skip_copyright
	lea data_base(pc),a5
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	move.l #DOS_FIB,d1
	moveq #0,d2
	CALL _DosBase(pc),AllocDosObject
	move.l d0,TMC_Fib-data_base(a5)
	beq no_fib

* On s'occuppe d'ouvrir correctement une fenetre
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	lea GadgetAttr(pc),a0			ouvre la topaz80
	...
	move.l sc_Font(a0),a1			barre de la fenetre
...
* Création d'une Checkbox : Split Module
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	lea TMC_NewGadget(pc),a1
	add.w #20,gng_TopEdge(a1)
	move.l #SplitText,gng_GadgetText(a1)
	addq.w #1,gng_GadgetID(a1)		ID=1

	move.l d0,a0				création d'une checkbox
	lea CheckBoxTags(pc),a2			Split Module
	move.l #CHECKBOX_KIND,d0
	CALL CreateGadgetA
	move.l d0,SplitGadget-data_base(a5)

* Création d'une CheckBox : Save SongName
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	lea TMC_NewGadget(pc),a1
	move.w #354,gng_LeftEdge(a1)
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	lea BevelTags(pc),a1
	move.l TMC_VisualInfo,4(a1)		GT_VisualInfo,(TMC_VisualInfo)
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	move.l TMC_Font(pc),a0			met la topaz80 comme font
...
Check_Which_Gadget
	...
	subq.w #1,d0
	beq SplitOpt_Change			ID=1	Split Module
	subq.w #1,d0
...
ExamineFile
	...
	move.l fib_Size(a0),FileSize-data_base(a5)
	...
	move.l FileSize(pc),d0			alloue de la mémoire pour
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Quit
	move.l _WorkbenchBase(pc),d0		la workbench.library est là ?
...
Load_Prefs
	...
	lea PrefsMsg,a0				de preferences
	bsr PrintText
...
No_Close_Prefs
	...
	eor.b #$ff,create_opt-data_base(a5)	kils ne se fassent pas
	eor.b #$ff,split_opt-data_base(a5)	inverser après...
...
* Fonction qui autorise le splitage du module TMC
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SplitOnFunction
	st split_opt-data_base(a5)
	bra read_pref_line

* Fonction qui interdit le splitage du module TMC
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SplitOffFunction
	sf split_opt-data_base(a5)
	bra read_pref_line
...
ScreenText	dc.b "The Module Converter (TMC) v4.1   (c)1993 Sync/DreamDealers",0
WindowText	dc.b "The Module Converter (TMC) v4.1",0
...
AboutMsg1	dc.b "The Module Converter v4.1"
AboutMsg1Size=*-AboutMsg1
AboutMsg2	dc.b "Coded and Copyright by Sync/DreamDealers"
AboutMsg2Size=*-AboutMsg2
AboutMsg3	dc.b "Released in July 1993"
AboutMsg3Size=*-AboutMsg3
...
Banner		dc.b 10
		dc.b "*******************************************************************",10
		dc.b "* Source-Song Generated With  TMC v4.1 ©1993 Sync of DreamDealers *",10
		dc.b "* From Module : "
NamePatch	dc.b "                                                  *",10
		dc.b "* SongName    : "
SongPatch	dc.b "                                                  *",10
